		...
		if (ioInfo->rwMode == READ)
		{
			...
			/*	printf("Rider 위치정보 : %s\n", ioInfo->buffer);*/
				...
				if (strstr(temp.c_str(), "Enrollment") != NULL) {
					...
					TcpValue.append("|");
					TcpValue.append(std::to_string(ntohs(addr_in->sin_port)));
			...
			//관리자 요청 처리
				...
				sprintf(ioInfo->buffer, sendString.c_str());
